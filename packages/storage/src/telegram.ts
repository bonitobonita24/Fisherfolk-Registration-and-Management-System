 ...
 * telegram.ts
 *
 * FRMS media storage via Telegram bot (T2 — see
 * docs/plans/telegram-storage-migration-plan.md §2/§4).
 *
 * Provides a lightweight, dependency-free mechanism to persist binary files
 * (fisherfolk photos/signatures, vessel photos, violation/ayuda evidence,
 * etc.) to a Telegram channel using the Bot API, and to retrieve them later
 * by file_id. Uses Node 22 global `fetch`, `FormData`, and `Blob` — no extra
 * npm packages required.
 *
 * Ported from Marine-Guardian's proven production implementation
 * (`packages/jobs/src/lib/telegram-storage.ts`) — logic kept verbatim,
 * MG-specific references (ER assets) generalized to FRMS media. This module
 * is intentionally standalone/prisma-free: `@frms/storage` never touches the
 ...
 * expected size of FRMS media (photos <=5 MB, signatures tiny, evidence
 * <=15 MB).
 *
 ...
 *     bytes: photoBytes,
 *     filename: "fisherfolk-photo.jpg",
 *     mimeType: "image/jpeg",
 *     caption: "calapan-city · fisherfolk-photo · calapan-city/fisherfolk-photo/abc123.jpg",
 *   });
 *   // Later …
 ...
 *
 * NOTE: Telegram bot getFile download is capped at 20 MB — well within FRMS
 * media limits. Files larger than 20 MB return an error from getFile (the
 * caller maps that to a clean non-200 response, never a crash).
 */
