/**
 * Unit tests — packages/storage/src/telegram.ts (T2)
 *
 ...
 *
 * See docs/plans/telegram-storage-migration-plan.md §10 T2.
 */
...
describe("telegram.ts", () => {
  ...
  describe("uploadDocumentToTelegram", () => {
    it("returns fileId from a successful sendDocument (document field)", async () => {
      ...
      const result = await uploadDocumentToTelegram({
        ...
        filename: "photo.jpg",
        mimeType: "image/jpeg",
        caption: "tenant-a · fisherfolk-photo · tenant-a/fisherfolk-photo/abc.jpg",
      });
  ...
  describe("fetchTelegramFileBytes", () => {
    ...
    it("honours retry_after on a 429 from getFile then succeeds", async () => {
      ...
      // Flush the backoff sleep (retry_after=1000ms, floor=500ms -> waits 1000ms).
