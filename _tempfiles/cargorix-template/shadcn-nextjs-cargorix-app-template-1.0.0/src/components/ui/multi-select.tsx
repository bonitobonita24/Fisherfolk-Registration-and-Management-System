...
const MultipleSelector = ({
  ...
  const onSearchSyncRef = React.useRef(onSearchSync)
  ...
  useEffect(() => {
    ...
    onSearchSyncRef.current = onSearchSync
  })
  ...
  useEffect(() => {
    ...
    const doSearchSync = () => {
      const res = onSearchSyncRef.current?.(debouncedSearchTerm)

      setOptions(transToGroupOption(res || [], groupBy))
    }

    const exec = async () => {
      if (!onSearchSyncRef.current || !open) return
  ...
  useEffect(() => {
    ...
    const doSearch = async () => {
      setIsLoading(true)
      const res = await onSearchRef.current?.(debouncedSearchTerm)
