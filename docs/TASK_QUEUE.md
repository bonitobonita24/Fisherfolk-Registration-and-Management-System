...
- 🔴 **FIS-12-RECONCILE — Reconcile FIS-12 migration drift (RELEASE-BLOCKER).** `EXPIRED` enum + 2 status migrations live only on unmerged `feat/fis12-registration-status-model` (`6892e64`), applied in dev DB, absent from this branch. Merge/rebase FIS-12 or cherry-pick the 2 migrations + enum before prod `migrate deploy`. `[WHAT]` in PENDING_DECISIONS. `owner 2026-09-01` `chore` `db`
