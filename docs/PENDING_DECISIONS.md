...
- **M5 demo-MinIO reclaim — CLOSED (won't do), owner decision 2026-07-16:** owner accepts the demo stack keeping its ~188 MB on MinIO (fixed demo dataset, won't grow over time; a demo account). Demo is NOT cut over to Telegram and its MinIO objects are NOT deleted.
- **STANDING STORAGE DEFAULT (owner 2026-07-16): Telegram is the default backend for ALL persistent media** (images/files/docs); MinIO only for throwaway/temp/index files. Applied per-env: **prod = Telegram** (done), **staging = Telegram** (wired 2026-07-16: live `.env` + SOPS `frms-staging-app.enc.env` @Server-Setups `13c7e2e`, bot @frms_assets_bot / channel `-1004351286489` — so prod→staging data-first refreshes render photos), **local dev = Telegram** (wired 2026-07-16 per owner: `.env.dev` `STORAGE_BACKEND=telegram`, same @frms_assets_bot but a DEDICATED private channel **`FRMS Dev Assets` / `-1004492567983`** so dummy dev uploads never pollute the prod/staging media channel; MinIO retained locally as temp/index fallback + still holds the source photo dataset), **demo = MinIO** (kept — the ONLY MinIO-backed env). **Target matrix: dev + staging + prod = Telegram; demo = MinIO.** Note: this is broader than the framework V32.16 default (MinIO-default) and also overrides the global `media-storage-default.md` "dev → MinIO" carve-out — that global carve-out reconciliation is a separate owner-gated fleet call.
