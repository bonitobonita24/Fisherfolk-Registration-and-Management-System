...
- **STANDING STORAGE DEFAULT (owner 2026-07-16): Telegram is the default backend for ALL persistent media** (images/files/docs); MinIO only for throwaway/temp/index files. Applied per-env: **prod = Telegram** (done), **staging = Telegram** (wired 2026-07-16: live `.env` + SOPS `frms-staging-app.enc.env` @Server-Setups `13c7e2e`, bot @frms_assets_bot / channel `-1004351286489` — so prod→staging data-first refreshes render photos), **local dev = MinIO** (kept — fast/local/self-contained; holds the source photo dataset), **demo = MinIO** (kept). Note: this is broader than the framework V32.16 default (MinIO-default) — fleet-wide/framework promotion is a separate owner-gated call.
