...
## 2026-07-01 — Prisma: RegistrationRenewal model + Fisherfolk.idReleasedAt/idReleasedById (S0)
- Agent:               CLAUDE_CODE (Swarm Worker S0, branch swarm/registration-status-timeline)
- Why:                 Phase 4 S0 — lay the DB foundation for the registration-status timeline feature. Adds the RegistrationRenewal audit table and the ID-release fields to Fisherfolk.
- Files modified:      packages/db/prisma/schema.prisma (new model RegistrationRenewal; Fisherfolk +idReleasedAt/idReleasedById/idReleasedBy/renewals; User +renewedRegistrations/idReleasedFisherfolk; Tenant +registrationRenewals)
...
## 2026-07-01 — List badge columns: RegistrationType (NEW/RENEWED) + ID-Release (S2)
- Agent:               CLAUDE_CODE (Swarm Worker S2, branch swarm/registration-status-timeline)
- Why:                 Phase 4 S2 — add the two new badge columns to the fisherfolk list: Registration Type (NEW=green / RENEWED=orange, derived from renewalCount===0) and ID Release (Released=green with date tooltip / Not Released=gray). WCAG 2.2 AA compliant (text labels, not color-only). Explicit color overrides bypass statusColorMap to avoid conflicts with the existing FisherfolkStatus column.
- Files modified:      apps/web/src/app/[tenant]/fisherfolk/columns.tsx (interface +idReleasedAt/renewalCount; +registrationType column; +idRelease column); apps/web/src/app/[tenant]/fisherfolk/fisherfolk-list-client.tsx (import FisherfolkListItem; explicit item mapping converting idReleasedAt Date→string via superjson-aware branch + _count.renewals→renewalCount)
- Code-review fix:     Replaced String(item.idReleasedAt) with direct pass-through (item.idReleasedAt) for the non-Date branch. String() on a Date that failed instanceof would produce a locale-dependent string causing unreliable new Date() parsing in the column cell.

## 2026-07-01 — Profile two-column shell + registration status + renewal history + Renew/MarkIdReleased buttons (S3)
- Agent:               CLAUDE_CODE (Swarm Worker S3, branch swarm/registration-status-timeline)
- Why:                 Phase 4 S3 — extend fisherfolk detail profile page with: two-column grid layout (LEFT main + RIGHT timeline slot); registration status line (NEW/RENEWED badge + date); Renewal History card; ID-Release status line (date + who); Renew Registration and Mark ID Released action buttons with RBAC gating (encoder/admin/super_admin via trpc.user.me) and violation-guard tooltip.
- Files modified:      apps/web/src/app/[tenant]/fisherfolk/[id]/fisherfolk-detail-client.tsx (two-column shell; registration status line; renewal history card; ID-release line; Renew + MarkIdReleased buttons via shared ConfirmDialog; WCAG: tooltip aria-label, non-empty aside landmark placeholder); apps/web/src/server/trpc/routers/fisherfolk.ts (getById include extended: +idReleasedBy{name,email} to support "Released by [name]" display)
- Code-review fixes:   (1) Used shared ConfirmDialog instead of inline Dialog — removes duplicate open/loading state and ensures dialog stays open on mutation error via re-throw pattern. (2) Added visible placeholder Card to <aside aria-label="Activity timeline"> to prevent empty WCAG landmark. (3) Dialog-on-error behavior: re-throw from onConfirm keeps ConfirmDialog open so user sees error in context.
...
- Files modified:      docs/DECISIONS_LOG.md (appended 2026-07-01 ID Generator entry with sub-decisions a–f + PM flag for 2 open [WHAT] questions); docs/IMPLEMENTATION_MAP.md (new Batch 4 — ID Generator section; Governance Docs + Locked Decisions counts updated); docs/STATE.md (current-state block updated); docs/CHANGELOG_AI.md (this entry).
- Verification:        git diff -- docs/PRODUCT.md confirms zero changes. Rule 1 preserved.

## 2026-07-01 — S3 (ID Generator wave): IDPrintBatch model (create-only migration) + idPrint router
- Agent:               CLAUDE_CODE (Swarm Worker S3, branch swarm/id-generator)
