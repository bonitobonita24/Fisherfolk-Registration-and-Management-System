...
import type { DataKey, TooltipValueType } from "recharts"
...
/** Safely converts a DataKey (which may be a function) or name to a plain string. */
function toKeyStr(
  key: DataKey<unknown> | string | number | undefined | null,
  fallback: string
): string {
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload ?? []
    const key = toKeyStr(labelKey ?? item?.dataKey ?? item?.name, "value")
    ...
    >
      ...
      <div className="grid gap-1.5">
        {(payload ?? [])
          ...
          .map((item, index) => {
            const key = toKeyStr(nameKey ?? item.name ?? item.dataKey, "value")
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
  ...
  return (
    ...
    >
      {(payload ?? [])
        .filter((item) => item.type !== "none")
        .map((item, index) => {
          const key = toKeyStr(nameKey ?? item.dataKey, "value")
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
