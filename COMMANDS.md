...
| `docker volume rm frms_dev_postgres_data` | Remove dev PostgreSQL volume only | YES — dev DB data |
| `docker volume rm frms_dev_valkey_data` | Remove dev Valkey volume only | YES — dev cache |
| `docker volume rm frms_dev_minio_data` | Remove dev MinIO volume only | YES — dev files |
| `docker volume ls` | List all Docker volumes | — |
| `docker image ls` | List all Docker images | — |
| `docker image prune -f` | Remove all dangling images | No |
| `docker image rm frms:dev-latest` | Remove specific image | No |
...
| `docker pull bonitobonita24/frms:staging-latest` | Pull staging image on staging server |
| `docker pull bonitobonita24/frms:latest` | Pull prod image on production server |
...
| `pnpm db:studio` | Open Prisma Studio at http://localhost:44397 (visual DB browser) |
...
| pgAdmin | http://localhost:44384 | See CREDENTIALS.md |
| MinIO Console | http://localhost:44381 | See CREDENTIALS.md |
| MailHog | http://localhost:44383 | No auth |
| Prisma Studio | http://localhost:44397 | No auth |
...
| `docker exec -it frms_dev_postgres psql -U frms_dev -d frms_dev` | Open PostgreSQL shell |
| `docker exec -it frms_dev_valkey valkey-cli` | Open Valkey (Redis) CLI |
| `docker logs frms_dev_app --tail 100` | Last 100 lines of app logs |
| `docker inspect frms_dev_app \| grep IPAddress` | Get container IP address |
| `pnpm --filter @frms/web dev` | Start only the web app (no Docker) |
| `pnpm turbo run build --filter=@frms/web` | Build only the web app |
